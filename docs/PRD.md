# Product Requirements Document (PRD): SIBI Smart Tutor

## 1. Executive Summary

**SIBI Smart Tutor** is an interactive, browser-based web application designed to democratize the learning of the formal Indonesian Sign Language (SIBI) alphabet. By utilizing client-side machine learning (TensorFlow.js), the application functions as a "smart mirror," analyzing user hand gestures via webcam and providing real-time corrective feedback. This tool targets students, educators, and inclusivity advocates, aiming to make SIBI learning accessible without specialized hardware.

## 2. Problem Statement

- **Lack of Accessibility:** Learning SIBI often requires in-person instruction or static 2D images, which are difficult to mimic accurately.
...
- **Accuracy:** Achieve >85% prediction accuracy for static alphabet signs (A-Z) under standard lighting conditions.
...
| **The Educator**              | A teacher looking for supplemental tools for the classroom. | Reliability, standard adherence to SIBI rules, and ease of deployment (just a URL). |
...
- **Gesture Classification:** A custom lightweight neural network (trained on SIBI datasets) to classify the vector data from the hand landmarks into alphabet letters (A-Z).
- **Visual Guide Overlay:** Display a semi-transparent "ghost" hand or wireframe guide over the camera feed showing the _correct_ pose for the target letter.
...
4. **Classification:** Keypoints are fed into the custom SIBI classifier model.
...
- **Ambiguity:** Similar signs (e.g., M vs. N in SIBI) require higher confidence thresholds or specific heuristic tie-breakers.
...
- **Split Screen:** On desktop, show the reference image (standard SIBI chart) on the left and the "Smart Mirror" (webcam feed) on the right.
